...
const strings = {
  no: {
    tagline: 'Rådgiver og «doer» innen teknologi, innovasjon og kommunikasjon',
    linksHeading: 'Lenker',
              ...
              {settings?.socialMedia?.map((social, i) => (
                <p key={i} className="flex items-center gap-2">
                  {social.platform?.toLowerCase() === 'linkedin' && (
                    <a href={social.url} className="shrink-0 text-[#0A66C2] hover:brightness-125 transition-[filter]" aria-label="LinkedIn">
                  ...
                  <a href={social.url} className="hover:text-white capitalize">{social.platform}</a>
                </p>
              ))}
            </address>
          </div>
