      ...
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center py-4">
          ...
          <nav className="hidden md:flex items-center gap-10 flex-1 justify-center">
            {links.map(({ href, label }) => (
              <Link key={href} href={href} className="text-white/90 hover:text-blue-600 transition-colors duration-200">
                {label}
              </Link>
            ))}
          </nav>
        ...
        {menuOpen && (
          <nav className="md:hidden flex flex-col gap-4 pb-4">
            {links.map(({ href, label }) => (
              <Link key={href} href={href} className="text-white/90 hover:text-blue-600 transition-colors duration-200" onClick={() => setMenuOpen(false)}>
                {label}
              </Link>
            ))}
          </nav>
